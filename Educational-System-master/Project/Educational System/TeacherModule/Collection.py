...
def Next_Btn_Click():
    try:
        ...
        if(quiz_title.get() and ques_points.get() and total_time.get()):
            ...
            conn.commit()
            cur.execute("Select Id from Quizzes where Name=? and Question_Numbers=? and Question_Mark=? and Total_Time=? and Teacher_Id=?"
                        ,[quiz_title.get().rstrip().lstrip(),int(Qnum.get()),int(ques_points.get()),int(total_time.get()),2])
            global Qout
            Qout = int(Qnum.get())
            global output
    ...
    if (ques_num_edt == Qout_edt - 1):
        for i in range(0,Qout_edt):
            ...
            conn.execute(
                ...
                ,[questionsEdt[i][0],questionsEdt[i][1],questionsEdt[i][2],questionsEdt[i][3],questionsEdt[i][4],questionsEdt[i][5],ffar_id,result[i][0]])
            conn.commit()
            conn.close()
...
Qnum_update=Spinbox(quiz_frame_update,values=["1","2","3","4","5"],state='disabled',bg="white",font=font1,width=80,textvariable=t_update)
